        ...
        for model_name, estimator in models.items():
            ...
            try:
                ...
                if model_name in [PAIRNET_NAME]:
                    ...
                    #### [DML estimation] ####
                    # estimator_tmp : outcome regression과 covariate representation (phi)를 위한 학습된함수
                    ...
                    # ATE
                    # PairNet+DML ATE : ATE : 4.010159492492676
                ...
                # 1. dml estimator separately -> WAAE : 3.08741815876354
                    ...
                    # PairNet WAAE with MuModel : WAAE for PairNet: 0.424854576587677
                    # pairnet_waae_mumodel = (np.abs(mu0_final.mean() - mu0_t.mean()) * ratio_D0) + (np.abs(mu1_final.mean() - mu1_t.mean()) * ratio_D1)
                    # print(f"WAAE for PairNet w/ Mu model: {pairnet_waae_mumodel}")
...
def cate_residualNet(phi_representation, y_t_w1, y_t_w0, cate_pred_out, idx_w0, idx_w1):
    ...
    # 최종 결과 확인
    mu1_final = mu1_model(X_tensor_1).detach() #517?
    mu0_final = mu0_model(X_tensor_0).detach() #1013?
